...
// ---- header -----
	...
	if ( ! empty( $bgimg ) ) {
		...
		$fit_height = arun_get_theme_option( 'fix_header_height' );
		...
		$himg_position = arun_get_theme_option( 'header_image_position' );
		switch ( $himg_position ) {
			case 'before':
				add_action( 'arun_header_top_wrap_begin', 'arun_the_header_image' );
				break;
			case 'after':
				add_action( 'arun_header_top_wrap_end', 'arun_the_header_image' );
				break;
	...
	if ( ! empty( $header_textcolor ) ) {
		$style .= apply_filters( 'arun_customizer_header_textcolor_css', "#logo{color:#$header_textcolor}" );
	}

	$main_color = arun_get_theme_option( 'maincolor' );
	if ( ! empty( $main_color ) && '#936' != $main_color && '#993366' != $main_color ) {
		...
		$style .= apply_filters( 'arun_customizer_main_color_css', $main_color_css );
	}

	$style = apply_filters( 'arun_customizer_css', $style );
	...
	echo ( $style )
		? "<!-- BEGIN Customizer CSS -->\n<style type='text/css' id='arun-customizer-css'>$style</style>\n<!-- END Customizer CSS -->\n"
		: "";

}

add_action( 'wp_head', 'arun_customizer_css' );
/* ======================================================================== */
...
// ------------------------
function arun_sanitize_text( $value ) {
	return sanitize_text_field( $value );
}


// ------------------------
function arun_sanitize_html( $value ) {
	return esc_html( $value );
}


// ------------------------
function arun_sanitize_textarea( $value ) {
	return esc_textarea( $value );
}


// ------------------------
function arun_is_single() {
	return is_single();
}

// ------------------------
function arun_is_page() {
	return is_page();
}

// ------------------------
function arun_is_singular() {
	return is_singular();
}

// ------------------------
function arun_is_default_layout() {
	return ! is_singular() && ! is_page() && ! is_home();
}

// ------------------------
function arun_show_on_home_posts( $control ) {
...
// ------------------------
function arun_custom_home_h1( $control ) {
...
if ( class_exists( 'WP_Customize_Control' ) ) {
	class arun_Group_Title_Control extends WP_Customize_Control {
 ...
 * ======================================================================== */
if ( ! function_exists( 'arun_customizer_live' ) ):
	function arun_customizer_live() {
		...
		wp_localize_script( 'arun-customizer-js', 'optname', ARUN_OPTION_NAME );

	}
endif;
add_action( 'customize_preview_init', 'arun_customizer_live' );

if ( ! function_exists( 'arun_customizer_control_toggle' ) ):
	function arun_customizer_control_toggle() {
...
add_action( 'customize_controls_enqueue_scripts', 'arun_customizer_control_toggle' );
/* ======================================================================== */
